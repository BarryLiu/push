...
<head>
    ...
    <script type="text/javascript">
        ...
        function getPagerUrl() {
        	var pagerUrl = "link/pushLinkServlet?1=1";
                ...
                if (v == 'ok') {
                    Login.Tip('precessing...', 'loading');
 					...
 					$.ajax({
				           cache:true,
				           type: "POST",
				           url:"${pageContext.request.contextPath}/link/pushLinkServlet",
    ...
    <div class="block">
        <div class="h">
            ...
            <div class="bar" style="display: block;">
                <a class="btn-lit" href="javascript:Index.Open('link/pushLinkServlet?operation=other&pageCount=${pagerInfo.pageCount }');"><span>Add</span></a>
                ...
                <table class="data-table" width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                        ...
                        <tr>
			              ...
			              <td class="txt100 c">
			              	<a href="javascript:Index.Open('link/pushLinkHisServlet?pushId=${userinfo.id }');" title="Detail">
			              		${userinfo.title }
			              	</a>	
			              </td>
			              <td class="txt c">
							<c:choose>
								<c:when test="${userinfo.linkType == 0 }">Website</c:when>
								<c:when test="${userinfo.linkType == 1 }">BookMark</c:when>
								<c:when test="${userinfo.linkType == 2 }">Notification</c:when>
								<c:otherwise>Unknown(${userinfo.linkType})</c:otherwise>
							</c:choose>
						  </td>
			              <td class="txt c">${userinfo.comments }</td>
			              <td class="txt100 c">${userinfo.countSended }</td>
			              <td class="txt100 c">${userinfo.countClicked }</td>
			              <td class="txt100 c">${userinfo.countSended + userinfo.countClicked }</td>
			              <td class="txt140 c">${userinfo.pushDate }</td>
			              <td class="icon">
			              <c:if test="${userinfo.canEdit }">
			              <a class="opt" title="Modify" href="javascript:Index.Open('link/pushLinkServlet?operation=other&'+J.getParam('${userinfo }'));"><span class="icon-sprite icon-edit"></span></a>
			              </c:if>
			              </td>
                          <td class="icon tail">
                          <!-- 

...
<script type="text/javascript">
		...
		$(function() {
			...
			var id = "${pushLink.id}";
			if(Check.isEmptyOrLenTooLong(url, 100, "url")) {
					J.focus($("#url"));
					return;
			}
			if(!urlCheck(url)){
			Login.Tip("Please enter the correct url!", "Worning");
			return;
			}
			...
			$.ajaxFileUpload({
				url:"${pageContext.request.contextPath}/link/pushLinkIconServlet",
				data:{title:title,linkType:linkType,pushDate:pushDate,url:url,comments:comments,model:model,
					...
					<form id="modifyForm">
						<table class="data-form" cellspacing="0" cellpadding="0">
							<tbody>
								...
								<tr id="uptr"
									<c:if test="${!empty pushLink.id }">style="display: none;"</c:if>>
								...
								<tr id="showtr"
									...
									<td><img id="icon"
										src="${pageContext.request.contextPath}/${pushLink.icon}"
										width="72px" height="72px" /> <a class="btn-lit"
										href="javascript:" id="reUpload"><span>Re-upload</span></a></td>
								</tr>
								<tr>
									<th scope="row">Url：</th>
									<td><input value="${pushLink.url }" type="text" name="url"
										id="url" class="input-normal" /> <font color="red">*</font></td>
								</tr>
								<tr>
									<th scope="row">Imei：</th>
									<td><input value="${pushLink.imei1}" type="text"
										name="imei1" id="imei1" class="input-small" maxlength="15" style="width:120px" /> ~ <input
										value="${pushLink.imei2}" type="text" name="imei2" id="imei2"
										class="input-small" maxlength="15" style="width:120px"/></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td><label><a id="submit"
										...
										value="${pushLink.countryName }" /> <input name="projectName"

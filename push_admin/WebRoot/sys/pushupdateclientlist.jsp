...
<head>
    ...
    <script type="text/javascript">
        ...
        function getPagerUrl() {
        	var pagerUrl = "sys/pushUpdateClientServlet?1=1";
            pagerUrl += "&title="+encodeURI(encodeURI('${requestScope.title}'))+"&countryName="+
            encodeURI(encodeURI('${requestScope.countryName}'))+"&type="+encodeURI(encodeURI('${requestScope.type}'));
                ...
                if (v == 'ok') {
                    Login.Tip('precessing...', 'loading');
 					...
 					$.ajax({
				           cache:true,
				           type: "POST",
				           url:"${pageContext.request.contextPath}/sys/pushUpdateClientServlet",
		...
		$(function() {
			$("#search").click(function() {
            	...
            	url = J.FormatString(url + '?title={0}&pageSize={1}&countryName={2}&type={3}', 
            	encodeURI(encodeURI(title)), pageSize,encodeURI(encodeURI(countryName)),encodeURI(encodeURI(type)));
            	Index.Open(url);
			});
...
<body>
<div class="container">
    <div class="location">Current Location：Update Client -&gt;Update Client</div>
        ...
        <div class="cnt-wp">
            <div class="cnt">
                <div class="search-bar">
                    ...
                    <label class="txt-green">Type：</label>
                    <select name="type" id="type" class="small" style="width: auto">
                    <option value="">-请选择-</option>
	                <option value="1" <c:if test="${type==1}">selected="selected"</c:if>>Bin Update</option>
	                <option value="2" <c:if test="${type==2}">selected="selected"</c:if>>Apk Update</option>
                    </select> 
    ...
    <div class="block">
        <div class="h">
            ...
            <h3>Update Client List</h3>
            <div class="bar" style="display: block;">
                <a class="btn-lit" href="javascript:Index.Open('sys/pushUpdateClientServlet?operation=other&pageCount=${pagerInfo.pageCount }');"><span>Add</span></a>
                ...
                <table class="data-table" width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                        ...
                        <tr>
			              ...
			              <td class="txt100 c">
			              	<a href="javascript:Index.Open('sys/pushUpdateClientHisServlet?pushId=${userinfo.id }');" title="Detail">
			              		${userinfo.title }
			              	</a>	
			              </td>
			              <td class="txt100 c">
			              <c:if test="${userinfo.type==1}">Bin Update</c:if>
			              <c:if test="${userinfo.type==2}">Apk Update</c:if>
			              </td>
			              <td class="txt c">${userinfo.comments }</td>
			              <td class="txt100 c">${userinfo.countSended }</td>
			              <td class="txt100 c">${userinfo.countInstalled }</td>
			              <td class="txt100 c">${userinfo.countSended+userinfo.countInstalled }</td>
			              <td class="txt140 c">${userinfo.pushDate }</td>
			              <td class="icon">
			              <c:if test="${userinfo.canEdit }">
			              <a class="opt" title="Modify" href="javascript:Index.Open('sys/pushUpdateClientServlet?operation=other&'+J.getParam('${userinfo }'));"><span class="icon-sprite icon-edit"></span></a>
			              </c:if>
			              </td>
                          <td class="icon tail">
                          <!-- 

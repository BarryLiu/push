...
Admin.Logout = function () {
    jBox.confirm('You sure you want to exit the system?', 'Prompt', function (v, h, f) {
        if (v == 'ok') {        
...
Admin.SubmitForm = function (name) {
		...
		var guestName=$('#guestName').trim();;//客户名称
		var prjName = $('#prjName').trim();//项目名称
		var mobileName=$('#mobileName').trim();//机器型号
		var hwarVersion=$('#hwarVersion').trim();//硬件版本
		var firstSwareVes=$('#firstSwareVes').trim();//初始软件版本
		var targetSwareVes=$('#targetSwareVes').trim();//目标软件版本
		var describe=$('#describe').trim();//描述
		var otaPath=document.getElementById("file").value;//OTA路径
		var priority=$('#priority').trim();//优先级
		var packageType=$('#packageType').trim();//包类型
		var editionType=$('#editionType').trim();//版本类别
		var ID=$('#ID').trim();;//ID
		 if (guestName == '') {
		        Login.Tip('Please Choose Customer Name!', 'warning');
		        $('#guestName').focus();
		        return;
	    }
	    if (prjName == '') {
	        Login.Tip('Please Choose Project Name!', 'warning');
	        $('#prjName').focus();
	        return;
	    }
	     if (mobileName == '') {
	        Login.Tip('Please Input Machine Model!', 'warning');
	        $('#mobileName').focus();
	        return;
	    }
	    if (hwarVersion == '') {
	        Login.Tip('Please Input Hardware Version!', 'warning');
	        $('#hwarVersion').focus();
	        return;
	    }
	    if (firstSwareVes == '') {
	        Login.Tip('Please Input Initial Version!', 'warning');
	        $('#firstSwareVes').focus();
	        return;
	    }
	    if (targetSwareVes == '') {
	        Login.Tip('Please Input Target Version!', 'warning');
	        $('#targetSwareVes').focus();
	        return;
	    }
	    if (describe == '') {
	        Login.Tip('Please Input OTA Describe!', 'warning');
	        $('#describe').focus();
	        return;
	    }
	    if(ID== ''){
	   // alert(otaPath);
	    	if(otaPath==''){
		    	 Login.Tip('Please Input Upgraded OTA Package!', 'warning');
		       	 $('#otaPath').focus();
		         return;
	    	}
	    	var reg=/[\u4E00-\u9FA5]/g;
			if (reg.test(otaPath)){
				 Login.Tip('No Chinese In Path of OTA!', 'warning');
		       	 $('#file').focus();
		         return;
			} 
	    }
	    if (priority == '') {
	        Login.Tip('Please Choose Priority!', 'warning');
	        $('#priority').focus();
	        return;
	    }
	    if (packageType == '') {
	        Login.Tip('Please Choose Package Type!', 'warning');
	        $('#packageType').focus();
	        return;
	    }
	     if (editionType == '') {
	        Login.Tip('Please Choose Version Type!', 'warning');
	        $('#editionType').focus();
	        return;
	    }
	    //Login.Tip('登录成功，正在跳转...', 'loading'); 		    
	    dwr.checkUpdateinfo(ID,guestName,prjName,mobileName,hwarVersion,firstSwareVes,targetSwareVes,function (data){
	    	if(data!=""){
	    	  Login.Tip(data+'！', 'warning');
	    	  return ;
	    	}
	    	else{
...
Login.Tip.ReLogin = function(code) {
	jBox.info("Please re-login system!", "Login outdated", { top: '200px', closed: code });
	//window.setTimeout(function(){jBox.close(true);}, 3000);
	window.setTimeout(function(){jBox.close(true);}, 500);
};
Login.Success = function(content, title, code) {
	jBox.success(content, title, { top: '200px', closed: code });
	//window.setTimeout(function(){jBox.close(true);}, 5000);
	window.setTimeout(function(){jBox.close(true);}, 500);
};
...
/* -----  Check ----- */
Check.isEmptyOrLenTooLong = function(val, len, name) {
	var text = null;
	if(J.isNullOrEmpty(val)) {
		text = "The "+name+" can not be empty!";
	} else if(val.length > len) {
		text = "The "+name+" length can not be larger than "+len+"!";
	}
	if(text != null) {
		jBox.tip(text, "error", { top: '100px' });
		return true;
	}
	return false;
};
...
Pager.Output = function (urlFormat, pageSize, pageIndex, pageCount, recordCount) {
    ...
    function _getLink(text, enabled, urlFormat, index) {
        if (enabled == false)
            ...
            return J.FormatString(' <a class="button-white" href="javascript:Index.Open(\'' + urlFormat + '\');"><span>{2}</span></a>', index,pageSize, text);
    }

    var html = [];
    html.push('<div class="pager-bar">');
    html.push(J.FormatString('<div class="msg">{0} records,{1}/{2} Current,{3} records per page</div>', recordCount, pageIndex, pageCount, pageSize));
    html.push(_getLink('Home Page', pageIndex > 1, urlFormat, 1));
    html.push(_getLink('Previous Page', pageIndex > 1, urlFormat, pageIndex - 1));
    html.push(_getLink('Next Page', pageCount > 0 && pageIndex < pageCount, urlFormat, pageIndex + 1));
    html.push(_getLink('Last Page', pageCount > 0 && pageIndex < pageCount, urlFormat, pageCount));
    html.push('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
    html.push('<input id="current-index" onkeydown="J.EnterSubmit(event, Pager.Jump);" class="input-small" style="text-align:center;" type="text" value="' + (pageIndex > 0 ? pageIndex : '') + '" />Page');
    html.push('&nbsp;&nbsp;&nbsp;&nbsp;');
    html.push('<a class="button-white"' + (pageCount <= 1 ? ' style="filter:Alpha(Opacity=60);opacity:0.6;" href="javascript:void(0);"' : ' href="javascript:Pager.Jump();"') + '><span>Go</span></a>');
    html.push('</div>');
...
Pager.Output1 = function (urlFormat, pageSize, pageIndex, pageCount, recordCount) {
    ...
    function _getLink(text, enabled, urlFormat, index) {
        if (enabled == false)
            ...
            return J.FormatString(' <a class="button-white" href="javascript:window.location.href=getFormatPagerUrl();"><span>{2}</span></a>', index,pageSize, text);
    }
...
Pager.Jump = function () {
    ...
    Index.Open(J.FormatString(Pager.Data.urlFormat, index, Pager.Data.pageSize));
};
